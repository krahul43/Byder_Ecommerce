...
const SubirTicket = ({ navigation, route }) => {
  const { imageTicket, orderIdGet, name, images, sizesArray, selectedSize, currentDate, current_price, product_id } = route.params
  ...
  const OrderData = () => {
    ...
    //     // AsyncStorage.setItem('orderid', response.order_id.toString());
    //     // AsyncStorage.setItem('orderproductid', response.product);
    //   }).catch((err) => {
    //     console.log(err)
    //   })
    // })
