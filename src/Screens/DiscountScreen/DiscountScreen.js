...
const Search = ({ data, navigation, route }) => {
  ...
  const [searchdataOld, setsearchDataOld] = useState([])
...
// bottomsheet subcategory data
  ...
  const product = () => {
    ...
    fetch(Baseurl + '/api/product/discount/', {
      ...
      .then((json) => {
        setLoaderVisible(false)
        setsearchData(json.slice(0, 30))
        setsearchDataOld(json.slice(0, 30))
          ...
          <TouchableOpacity onPress={() => navigation.navigate('DetailSearch', { item,distace, data, ScreenNavigation: "Search" })}>
            <View style={{ marginLeft: hp('2%') }}>
              ...
              <View style={styles.maintx}>
                <Text style={[styles.txtclr,{fontSize:hp('1.7%')}]}>{item.area}</Text>
