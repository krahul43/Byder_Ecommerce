...
const RelatedProducts = ({ navigation, route, data, onHeartPress }) => {
  const { item, ScreenNavigation,distace } = route.params
  ...
  //     // Call the shared onHeartPress function passed as a prop to update the liked status in the parent component
  //     onHeartPress(item.product_id);
  //   };
  ...
  // Update the current date in state every second
  useEffect(() => {
  ...
  return (
    <>
      ...
      <SafeAreaView>
        <Topbarback Textheading={item.name.slice(0, 16)} navigation={navigation} />
