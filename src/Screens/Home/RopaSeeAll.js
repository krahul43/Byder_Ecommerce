...
const RopaSeeAll = ({ data, navigation }) => {
  ...
  const [ropaOldData, setRopaOldData] = useState([])
  console.log(ropaData, 'ropaData')
  const RopaData = () => {
  ...
  const Search = txt => {
    if (txt !== '') {
      let tempData = ropaData.filter(item => {
      ...
      setRopaData(tempData);
    } else {
      setRopaData(ropaOldData);
    }
  };
    ...
    <>
      <SafeAreaView style={styles.MainFlx2}>
        ...
        <FlatList
          ...
          data={ropaData} // Use the filtered data instead of `searchdata`
