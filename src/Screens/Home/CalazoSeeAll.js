import { StyleSheet, Text, View, StatusBar, Image, ImageBackground, SafeAreaViewBase, TextInput, FlatList, TouchableOpacity, SafeAreaView } from 'react-native'
...
const CalazoSeeAll = ({ data, navigation }) => {
  ...
  const [calzadoOldData, setcalzadoOldData] = useState([])
  console.log(calzadoData, 'calzadoData')
  const CalzadoData = () => {
  ...
  const Search = txt => {
    if (txt !== '') {
      let tempData = calzadoData.filter(item => {
      ...
      setcalzadoData(tempData);
    } else {
      setcalzadoData(calzadoOldData);
    }
  };
...
<View style={{padding:5,marginTop:8}}>
      <View style={styles.maincard}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('DetailSearch', { item,distace })}>
          <View style={{ marginLeft: hp('2%') }}>
            ...
            <Text style={styles.txtclr} umberOfLines={2}>{item.name.slice(0, 14)}...</Text>
  ...
  return (
    <>
      ...
      <SafeAreaView style={styles.MainFlx}>
        <View style={{marginLeft:wp('-0.2%')}}>
        ...
        <FlatList
          ...
          data={calzadoData} // Use the filtered data instead of `searchdata`
